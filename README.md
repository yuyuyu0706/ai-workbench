...
PromptTrail は Vitest と React Testing Library を使い、Node.js 上の jsdom 環境でコンポーネントの意味構造と表示内容を検証します。初期テストは静的な Welcome Page のスモークテストに限定し、Router、IndexedDB、API mock、カバレッジ、ブラウザ E2E は後続テーマで必要になってから追加します。
