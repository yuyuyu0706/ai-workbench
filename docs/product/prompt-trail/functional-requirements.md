...
| 作成・編集          | Phase 2でPrompt種別、タイトル、本文の順に入力し、Page Headerとform下部の同一formに接続された保存Actionから登録・更新できる。Prompt Editorのフィールドラベルは「種別」「タイトル」と表示し、種別とタイトルは横並びgridで配置する。Prompt本文ラベル右のコピーボタンでtextareaの現在入力値をクリップボードへコピーできる。本文に`${varName}`形式の変数プレースホルダーが含まれる場合はラベル行に変数バッジを表示し、コピーボタンクリックで変数入力パネルを開いて値を入力後にコピーできる。未入力の変数は`${varName}`のままコピーされる。Prompt本文textareaは種別・タイトル欄（最大44rem）より横幅を広くとり、カード幅いっぱいまで拡大する                                                                                              |
...
| 反復利用            | Libraryの同一Prompt資産を参照して複数のRunを作り、作成時点の内容を毎回Snapshotとして固定する。本文全文は右側優先配置の非modal Popoverで確認し、Popover内で本文だけをinline編集できる。未保存draftがある間はFilter／Search／Sort／条件クリアを無効化し、別Prompt、編集、新規登録、Trail作成などの遷移は破棄確認を経由する。保存は`expectedUpdatedAt`付きの本文限定atomic更新で、stale時はdraftを保持して最新本文を読み込んでから再保存し、not-found／unavailable時は保存を止めてclose後に一覧を再読込する。HeaderのPrompt全体編集Link／copy icon／close iconから操作する。閲覧modeでは本文に`${varName}`形式の変数プレースホルダーが含まれる場合、本文の直前に変数バッジを表示し、copy iconクリックでPopover内に変数入力パネルを開いて値を入力してからコピーできる。未入力の変数は`${varName}`のままコピーされ、コピー自体はブロックされない。パネルを開くと最初の入力欄へ、閉じるとcopy iconへフォーカスが移動する。パネルの値は非永続で、コピー完了・編集modeへの切替・Popover全体を閉じたタイミングでリセットされる。Esc・外側クリックは既存のPopover全体クローズ挙動のままとする。Trail作成はTooltipとPrompt名入りAccessible Nameを備えたPrimary icon Linkとする。狭幅ではTable regionだけを横scrollし、Domain／Repository／DB契約は変えない |
