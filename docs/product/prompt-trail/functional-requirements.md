...
| 作成・編集          | Phase 2でPrompt種別、タイトル、本文の順に入力し、Page Headerとform下部の同一formに接続された保存Actionから登録・更新できる。Prompt Editorのフィールドラベルは「種別」「タイトル」と表示し、種別とタイトルは横並びgridで配置する。Prompt本文ラベル右のコピーボタンでtextareaの現在入力値をクリップボードへコピーできる。本文に`${varName}`形式の変数プレースホルダーが検出されると、クリック不要で常に変数入力パネル（各変数の入力欄＋コピーボタン）を表示する。コピーボタンクリック時はパネルの現在値を使って置換・コピーする挙動と統一する。未入力の変数は`${varName}`のままコピーされる。Prompt本文textareaは種別・タイトル欄（最大44rem）より横幅を広くとり、カード幅いっぱいまで拡大する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
