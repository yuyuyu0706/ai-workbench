...
| 反復利用            | Libraryの同一Prompt資産を参照して複数のRunを作り、作成時点の内容を毎回Snapshotとして固定する。本文全文は右側優先配置の非modal Popoverで確認し、Popover内で本文だけをinline編集できる。未保存draftがある間はFilter／Search／Sort／条件クリアを無効化し、別Prompt、編集、新規登録、Trail作成などの遷移は破棄確認を経由する。保存は`expectedUpdatedAt`付きの本文限定atomic更新で、stale時はdraftを保持して最新本文を読み込んでから再保存し、not-found／unavailable時は保存を止めてclose後に一覧を再読込する。HeaderのPrompt全体編集Link／copy icon／close iconから操作する。閲覧modeでは本文に`${varName}`形式の変数プレースホルダーが検出されると、本文の直前に常に変数入力パネル（各変数の入力欄＋コピーボタン）を表示する。copy iconクリック時はパネルの現在値を使って置換・コピーする挙動と統一する。未入力の変数は`${varName}`のままコピーされ、コピー自体はブロックされない。パネル表示・非表示の切替時に本文表示へ不用意にフォーカスは移動せず、変数が0件になりパネルが消える際にパネル内へフォーカスがあった場合のみedit iconへフォーカスを戻す。パネルの値は非永続で、コピー完了・編集modeへの切替・Popover全体を閉じたタイミングでリセットされる。Esc・外側クリックは既存のPopover全体クローズ挙動のままとする。Trail作成はTooltipとPrompt名入りAccessible Nameを備えたPrimary icon Linkとする。狭幅ではTable regionだけを横scrollし、Domain／Repository／DB契約は変えない |
