...
| 機能                | 要件・Phase配置                                                                                                                                                                                            |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一覧                | Phase 2でPrompt Libraryを実データへ接続し、更新日時順を維持したcompactなSemantic tableでPrompt名、Project、種別、更新日時、Prompt本文表示、操作を比較できる                                                |
...
| 反復利用            | Libraryの同一Prompt資産を参照して複数のRunを作り、作成時点の内容を毎回Snapshotとして固定する。本文全文は右側優先配置の非modal Popoverで確認・コピーし、Prompt名を編集導線、Trail作成をPrimary Actionとする |
