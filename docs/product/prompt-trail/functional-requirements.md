...
| 機能                | 要件・Phase配置                                                                                                                                                   |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一覧                | Phase 2でPrompt Libraryを実データへ接続し、更新日時順に表示する。タイトル、種別、Scope、更新日時、本文Previewの順で示し、Desktopでは複数列、狭幅では1列で表示する |
...
| 簡易検索            | Phase 2でタイトルと本文をキーワード検索できる。全件数と表示件数を区別し、検索結果0件では条件変更と検索解除を案内する                                              |
| 論理削除            | Phase 2で確認後に`deletedAt`を設定し、Libraryと新規Trail作成から除外する。過去Run、Link、Prompt Snapshotは維持する                                                |
| 反復利用            | Libraryの同一Prompt資産を参照して複数のRunを作り、作成時点の内容を毎回Snapshotとして固定する。カードではTrail作成をPrimary、編集をSecondary Actionとする          |
