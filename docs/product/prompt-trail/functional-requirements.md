...
| 反復利用            | Libraryの同一Prompt資産を参照して複数のRunを作り、作成時点の内容を毎回Snapshotとして固定する。本文全文は右側優先配置の非modal Popoverで確認し、Headerの編集Link／copy icon／close iconから操作する。Trail作成はTooltipとPrompt名入りAccessible Nameを備えたPrimary icon Linkとする。狭幅ではTable regionだけを横scrollし、Domain／Repository／DB契約は変えない |
