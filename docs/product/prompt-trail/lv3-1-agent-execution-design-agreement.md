...
[closed]`による自動起動は設計の選択肢として記録するが、P3-5では実装しない。
- 理由：「P3-5は各STEPが個別に動作するところまで、UIからの連結はP3-6」という#323で合意済みの
  境界と整合させるため。自動起動を入れるとP3-5がP3-6のスコープへはみ出す。
...
- #270本文のP3-6の「問い」を、P3-5／P3-6の分担（P3-5：STEP4〜10が個別にエージェント実行として
  動作しRunとして記録される状態まで／P3-6：それらをUIから連結してワンストップで回す体験）に
  合わせて書き換える必要がある。
...
- STEP9/10の自動起動、UIからのエージェント実行導線（いずれもP3-6）。
...
- [ ] #270本文のP3-6「問い」の書き換え、「未割当の論点：文脈取得能力」への追記が行われている
