...
| GitHub Pages Deploy          | `codex/**` push / manual dispatch | Development Preview 配信 | 開発中成果を Shared Development Preview へ配信できた |
| Azure Static Web Apps Deploy | push `main`                       | Public Preview 配信      | `main` 統合版を Public Preview へ配信できた          |
...
この checklist は、後続 Lv4 の Hosted Preview 最終受入で利用する未実施の受入基準です。本 Issue では checklist を定義するだけで、実際の hosted 実機確認と完了判定は行いません。
...
- `.github/workflows/azure-static-web-apps-red-flower-0ff1f6100.yml` の変更。
- workflow 名変更・コメント整理。
