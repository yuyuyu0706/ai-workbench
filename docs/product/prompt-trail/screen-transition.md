...
対象時点は **P1-1-1-2 完了時点**です。BrowserRouter、AppShell、AppRouter により、Dashboard、Prompt Library、Context Library、Recipe Builder、Run Detail、Not Found の画面へ到達できます。Dashboard は Repository 実データ接続済みで、`loading` / `empty` / `failure` / `data` を現行状態として表示します。Prompt Library、Context Library、Recipe Builder は静的 start state、Run Detail は route parameter を受ける静的骨格です。Not Found は Dashboard への回復導線を持つ recovery route として維持します。
...
P1-1-1-2 完了時点の PromptTrail は、`/` から `/dashboard` へ redirect し、Dashboard を基本入口として利用者が AI 作業を再開する構成です。AppShell 配下では各画面へ到達でき、主要 4 画面のグローバルナビゲーションと Dashboard への復帰導線を通じて移動できます。Dashboard は Repository の実データを表示し、Prompt Library、Context Library、Recipe Builder、Run Detail は P0-4-3 で確立した静的骨格を現在も維持します。
...
この節は P0-4-3 で確立した静的 Page Skeleton の設計経緯です。P0-5 では Dashboard のみ Repository 実データ表示へ更新されましたが、Prompt Library、Context Library、Recipe Builder、Run Detail はここで定めた静的骨格を維持しています。画面ごとの本格 CRUD、検索、Recipe 実行、Dashboard 以外の Repository 連携は Phase 0 の対象外です。
...
Dashboard includes a persistent **新しいTrailを始める** CTA. It opens the contextual `/runs/new` route, where a prompt body creates the default Project, Prompt, and Direct Run before navigating to `/runs/:runId`. Run Detail displays the saved prompt snapshot and allows HTTP(S) Link registration in the same Trail.

## P1-1-1-2 現行導線

Dashboard の「新しいTrailを始める」CTA は global navigation に含めない contextual route `/runs/new` へ遷移します。Prompt を保存すると Direct Run Detail へ移動します。Run Detail は Repository 接続済みで、loading / not-found / failure / data を表示し、Prompt Snapshot、Recipe Run の Context Snapshot、HTTP(S) Link 登録と登録済み Link を確認できます。静的 Run Detail 骨格の説明は P0-4-3 時点の経緯であり、現行仕様ではありません。
