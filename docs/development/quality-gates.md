...
この文書は、AI Workbench / PromptTrail の品質ゲートと開発運用に関する唯一の正本です。root `README.md` は日常コマンドへの入口、`apps/prompt-trail/README.md` は PromptTrail 固有の入口として扱います。環境構築から preview までの正常系手順は [ローカル開発](local-development.md) を正とし、品質判断と失敗時の初動は本書を優先します。症状別の詳細診断・復旧は[環境・起動・品質ゲートのトラブルシューティング](troubleshooting.md)を正とします。
...
- Install、Lint、Format Check、Unit Test、Chromium 導入、E2E、Build の失敗時の短い初動と、詳細診断への導線。
...
この表は、マージ判断のための短い初動です。原因の絞り込み、結果別の復旧、禁止する回避策は[環境・起動・品質ゲートのトラブルシューティング](troubleshooting.md)を参照してください。

| ゲート               | 初動                                                                     | 再実行コマンド                                | 詳細診断                                                                                                                                           |
| -------------------- | ------------------------------------------------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Install dependencies | Node.js / pnpm version、lockfile 差分、registry 接続先を確認する。       | `pnpm install --frozen-lockfile`              | [Runtime](troubleshooting.md#5-runtime--nodejs--pnpm--corepack)、[Dependency Install](troubleshooting.md#6-dependency-install--lockfile--registry) |
| Lint                 | 最初に失敗したファイル、rule、warning を確認する。                       | `pnpm lint`                                   | [Lint / Format](troubleshooting.md#8-lint--format-check)                                                                                           |
| Format Check         | 対象ファイルを確認し、必要な場合だけローカルで整形して diff を確認する。 | `pnpm format:check`                           | [Lint / Format](troubleshooting.md#8-lint--format-check)                                                                                           |
| Unit Test            | 失敗した利用者向け表示と jsdom setup を確認する。                        | `pnpm test`                                   | [Unit / Component Test](troubleshooting.md#9-unit--component-test)                                                                                 |
| Chromium 導入        | CDN、DNS、Proxy、TLS、OS 依存の失敗箇所を確認する。                      | `pnpm --filter prompt-trail test:e2e:install` | [Playwright Chromium](troubleshooting.md#10-playwright-chromium-導入)                                                                              |
| E2E                  | Desktop / Mobile、4173、Vite 起動、artifact を確認する。                 | `pnpm test:e2e`                               | [Browser E2E](troubleshooting.md#11-browser-e2e)                                                                                                   |
| Build workspace      | 最初の TypeScript / Vite error を確認する。                              | `pnpm build`                                  | [production build / preview](troubleshooting.md#12-production-build--preview)                                                                      |

外部ネットワーク、GitHub Actions hosted runner、Playwright CDN、Ubuntu package source などが原因と疑われる場合も、コード不備と混同せず、失敗ログ、接続先、HTTP status、同じ commit での再実行前後の結果を PR に記録します。
