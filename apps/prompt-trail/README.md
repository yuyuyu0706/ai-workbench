...
どちらも Vite 開発サーバーを起動します。既定 URL は `http://localhost:5173/` です。通常の `pnpm dev` では、ポートが使われている場合に Vite が代替ポートを表示するため、ログの `Local:` に表示された URL をブラウザで開いてください。停止は `Ctrl+C` です。
...
preview は既定では `http://localhost:4173/` を表示します。通常の `pnpm --filter prompt-trail preview` では、4173 が利用中の場合に Vite が代替ポートを提示します。その場合は Vite ログの `Local:` URL を正とします。停止は `Ctrl+C` です。
...
`pnpm --filter prompt-trail test:e2e` は Playwright 設定により `http://127.0.0.1:4173/` を `strictPort` で固定使用して Vite dev server を起動し、Desktop Chromium と Pixel 5 相当 Mobile Chromium の 2 project を実行します。ローカルでは同じ URL の既存サーバーを再利用できますが、4173 が別用途で利用中の場合は代替ポートへ切り替わらず失敗します。4173 が競合した場合は、既存プロセスが PromptTrail の開発サーバーかを確認し、別用途なら停止してから再実行してください。CI では既存サーバーを再利用せず新規起動します。失敗時のみ trace と screenshot を残します。CI の artifact は `apps/prompt-trail/playwright-report/` と `apps/prompt-trail/test-results/` を確認してください。
...
- **E2E が失敗する**: `pnpm --filter prompt-trail test:e2e` を再実行し、固定ポート `4173` の競合、Vite dev server の起動、Desktop / Mobile project の失敗差分、`playwright-report/` と `test-results/` の trace / screenshot を確認します。4173 が競合している場合は、既存プロセスが PromptTrail の開発サーバーかを確認し、別用途なら停止してから再実行します。
...
- **5173 / 4173 が使えない**: 通常の `pnpm dev` と `preview` は、Vite が代替ポートを表示した場合にログの `Local:` URL を開きます。一方、`pnpm test:e2e` は `127.0.0.1:4173` 固定かつ `strictPort` のため、4173 が競合すると代替ポートへ切り替わらず失敗します。
