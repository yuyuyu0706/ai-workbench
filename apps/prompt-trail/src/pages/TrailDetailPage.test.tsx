...
/** Common repository deps for a single-Trail / single-Step / single-Run page. */
function baseTrailDeps(
  overrides: {
    trail?: any;
    project?: { name: string };
    steps?: readonly any[];
  ...
  const resolvedProject = overrides.project ?? { name: 'Project' };
  const resolvedSteps = overrides.steps ?? [trailStep];
  const resolvedRuns = overrides.runs ?? [direct];
...
describe('TrailDetailPage', () => {
  ...
  it('ignores an old stale reload after switching to another Trail', async () => {
    const trailA = {
      ...trail,
      id: 'trail-a',
      projectId: 'project-a',
    ...
    const stepA = { ...trailStep, id: 'step-a', trailId: 'trail-a' };
    const stepB = { ...trailStep, id: 'step-b', trailId: 'trail-b' };
    const runA = {
      ...direct,
      id: 'run-a',
      projectId: 'project-a',
    ...
    const repository = {
      ...
      listStepsByTrail: vi.fn(async (trailId: string) =>
        trailId === 'trail-b' ? [stepB] : [stepA],
      ),
  ...
  it('shows not-found only when the Trail itself is missing', async () => {
  ...
  it('shows the Trail (not not-found) and an empty state when it has zero Steps', async () => {
    const repository = {
      ...baseTrailDeps({ steps: [], runs: [] }),
    } as any;
    renderPage(repository);
  ...
  it('renders Direct Run without Recipe and Recipe Run context', async () => {
    const repo = { ...baseTrailDeps(), getRecipe: vi.fn() } as any;
    renderPage(repo);
  ...
  it('formats summary dates and preserves their original values', async () => {
    ...
    const repository = {
      ...baseTrailDeps({ runs: [runWithDates] }),
    } as any;
    renderPage(repository);
...
describe('TrailDetailPage Step list', () => {
  it('renders Steps in order ascending', async () => {
    const step1 = { ...trailStep, id: 'step-1', order: 1, title: 'First step' };
    const step2 = {
      ...trailStep,
      id: 'step-2',
      order: 2,
    ...
    const repository = {
      ...baseTrailDeps({ steps: [step1, step2], runs: [] }),
    } as any;
    renderPage(repository);

    const cells = await screen.findAllByText(/^\d\. /);
  ...
  it('shows manual Steps as title-led with no actions', async () => {
    const manualStep = {
      ...trailStep,
      kind: 'manual',
      promptId: null,
    ...
    const repository = {
      ...baseTrailDeps({ steps: [manualStep], runs: [] }),
    } as any;
    renderPage(repository);

    expect(await screen.findByText('1. Manual review')).toBeInTheDocument();
    expect(screen.getByText('人手の工程')).toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /表示|実行する|やり直す/ }),
    ).not.toBeInTheDocument();
  });

  it('shows only the Prompt icon for a Run-less prompt Step, heading "Prompt"', async () => {
    const user = (await import('@testing-library/user-event')).default.setup();
    const repository = {
      ...baseTrailDeps({ runs: [] }),
    } as any;
    ...
    expect(
      screen.queryByRole('button', { name: '関連リンクを表示' }),
    ).not.toBeInTheDocument();
    const promptButton = screen.getByRole('button', { name: 'Promptを表示' });
    await user.click(promptButton);
    expect(screen.getByText('Prompt', { selector: 'h3' })).toBeVisible();
    expect(screen.getByText('Prompt A body')).toBeVisible();
  });

  it('shows "削除済みのPrompt" when the referenced Prompt is missing', async () => {
    const repository = {
      ...baseTrailDeps({ runs: [], prompt: null }),
    } as any;
    renderPage(repository);

    expect(await screen.findByText('削除済みのPrompt')).toBeInTheDocument();
  });

  it('keeps the Trail-info edit button visible when the Trail is not deleted', async () => {
    const repository = { ...baseTrailDeps() } as any;
    renderPage(repository);
...
describe('TrailDetailPage Link form', () => {
  it('rejects an empty Link title without saving', async () => {
    const user = (await import('@testing-library/user-event')).default.setup();
    const repository = { ...baseTrailDeps(), saveLink: vi.fn() } as any;
    renderPage(repository);
    await screen.findByText('Prompt A');
    await openLinksPopover();
...
it('keeps Run B state when a pending Run A Link save resolves after a route change', async () => {
  ...
  const repository = {
    ...
    getPrompt: vi.fn(async (id: string) => ({
      ...prompt,
      id,
      title: id === 'prompt-b' ? 'Prompt B' : 'Prompt A',
    })),
...
describe('TrailDetailPage Run actions popovers', () => {
  ...
  it('shows the Run conversation inside the result popover once executed', async () => {
    ...
    const repository = {
      ...baseTrailDeps({ runs: [executedRun] }),
    } as any;
    renderPage(repository);
    await openResultPopover();
  ...
  it('continues the conversation by sending a follow-up message', async () => {
    ...
    const repository = {
      ...baseTrailDeps({ runs: [runWithHistory] }),
...
describe('Step reorder/delete (issue #342)', () => {
  function twoSteps() {
    return [
      { ...trailStep, id: 'step-1', order: 1, title: 'First step' },
      { ...trailStep, id: 'step-2', order: 2, title: 'Second step' },
    ];
  }
  ...
  it('swaps the order when "下へ" is clicked from the edit popover, and keeps the popover open', async () => {
    ...
    const repository = {
      ...baseTrailDeps({ steps, runs: [] }),
      reorderTrailSteps: vi.fn(async ({ orderedStepIds }) => {
  ...
  it('removes the row after confirming delete from within the edit popover, renumbers remaining Steps, and moves focus to "Stepを追加"', async () => {
    ...
    const repository = {
      ...baseTrailDeps({ steps, runs: [] }),
      softDeleteTrailStep: vi.fn(async ({ trailStepId }) => {
        const remaining = steps.filter((step) => step.id !== trailStepId);
        steps.splice(0, steps.length, ...remaining);
        return { ...trailStep, id: trailStepId, deletedAt: '2026-01-02' };
      }),
      listStepsByTrail: vi.fn(async () =>
        steps.map((step, index) => ({ ...step, order: index + 1 })),
      ),
    } as any;
    renderPage(repository);
    ...
    await waitFor(() =>
      expect(repository.softDeleteTrailStep).toHaveBeenCalled(),
    );
    await waitFor(() =>
      expect(screen.queryByText(/First step/)).not.toBeInTheDocument(),
    );
    expect(screen.getByText('1. Second step')).toBeInTheDocument();
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Stepを追加' })).toHaveFocus(),
    );
  });

  it('shows a notice and reloads on a stale reorder result', async () => {
    const user = (await import('@testing-library/user-event')).default.setup();
    const steps = twoSteps();
    const repository = {
      ...baseTrailDeps({ steps, runs: [] }),
  ...
  it('disables row edit icons while the add form is confirming discard', async () => {
    const user = (await import('@testing-library/user-event')).default.setup();
    const repository = {
      ...baseTrailDeps({ runs: [] }),
    } as any;
  ...
  it('disables the add button while the row edit form is confirming discard', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Stepを編集' }));
    await user.clear(screen.getByLabelText('Step名'));
    await user.type(screen.getByLabelText('Step名'), 'Draft edit');
    await user.click(screen.getByRole('button', { name: 'キャンセル' }));
    expect(screen.getByText('入力内容を破棄しますか？')).toBeInTheDocument();
