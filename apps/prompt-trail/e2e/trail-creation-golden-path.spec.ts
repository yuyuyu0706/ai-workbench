...
async function expectCreatedTrail(page: Page) {
  ...
  await expect(promptPopover.locator('pre')).toHaveText(promptBody);

  const linkPopover = await openLinksPopover(page);
  const savedLink = linkPopover.getByRole('listitem');
